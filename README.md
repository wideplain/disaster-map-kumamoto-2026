...
- **[内閣府 防災情報 令和8年熊本地震](https://www.bousai.go.jp/updates/r8kumamoto_jishin/index.html)**
  ...
  - 時系列の各時点は原則として県資料が骨格だが、**県が資料を出さない日（8/12など）は内閣府報だけの時点**を作る
    ...
    その時点の市町村別は断水のみ（内閣府報に市町村別があるのは断水だけ）。県合計は避難所・避難者・断水に加え、
    同じ報の**消防庁ベースの熊本県全体値**（死者・負傷者・住家被害）を入れる。ただし県資料とは集計基準が違い
    （負傷者は重傷/軽傷等、住家被害は床上・床下浸水を含む）、前後の時点と数値が大きく食い違うため、
    `summary_source` に出典を残してUI側で必ず注記を出す。
    消防庁表のパースは列の位置合わせではなく「住家合計＝内訳の和」「人的合計＝死者＋負傷者小計」が
    成り立つ並びを探す方式で、成り立たない報では値を返さない（誤った数値を出すより持たない方を選ぶ）。
...
- **[熊本県 令和8年熊本地震に関する情報](https://www.pref.kumamoto.jp/soshiki/1/274517.html)**（広報課の総合ポータル）から辿れる生活支援ページ群 → `parse_support.py`（**支援拠点マップ**の元データ）
  ...
  - [建設型応急住宅の進捗状況](https://www.pref.kumamoto.jp/soshiki/117/275490.html)（団地名・戸数・構造・着手日・入居予定・施工者）。
    ...
    （2）各報道資料PDF内の「新たに工事着手する仮設住宅団地の概要」ミニ表から、まだ一覧に無い団地だけを団地名の一致
    （`housing_team_key()`。NFKC正規化に加え、初期のHTML表だけに残る市町村名プレフィックスの有無も吸収する）で追加し、
    （3）同じPDFの「建設戸数を追加する団地概要」（第2期増戸）表から、既存団地の戸数を更新する。表が復活しない限りこの経路が唯一の更新手段になる。
    増戸は`unit_history`（発効日ごとの戸数チェックポイント）に積み、`build_timeline.py`は時点ごとにその時点で有効な戸数を引く
    （そうしないと後日の増戸が過去の時点にまで遡って反映されてしまう）。
